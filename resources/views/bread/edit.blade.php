    ...
    <div class="row">
        <div class="col-md-12">
            <div class="panel panel-dark panel-bordered">
                ...
                <form class="myform" role="form" action="{{ route('voyager.'.$dataType->name.'.update', $data->id) }}"
                        ...
                        <div class="form-group @if($row->type == 'hidden') hidden @endif col-md-{{ $display_options->width ?? 12 }} {{ $errors->has($row->field) ? 'has-error' : '' }}"
                            ...
                            @switch($row->type)
                                ...
                                @case('code_editor')
                                    ...
                                    <textarea name="{{ $row->field }}" id="{{ $row->field }}" class="hidden code_editor">{{ $data->$myfield }}</textarea>
                                    @break
                            @endswitch
                        </div>
                        @endforeach
                    ...
                    <div class="form-group text-center">
                        <hr />
                        <button type="submit" id="button_submit" class="btn btn-sm  btn-primary"><i
                                class="voyager-edit"></i> {{ __('voyager::generic.edit') }}</button>
                        <button type="button"
                            onclick="ajax('{{ route('voyager.'.$dataType->name.'.show', $dataType->id) }}', 'get')"
                            class="btn btn-sm btn-success"><i class="voyager-double-left"></i>Cancelar</button>
                    </div>
                ...
                <form id="form-update-img" action="#" method="post">
                    @csrf
